...
namespace OperatorsOverloading
{
    ...
    public abstract class List<TValue>
    {
        ...
        {
            if(list1.Length != list2.Length || list1 is null || list2 is null)
            {
                return false;
            }
        ...
        /// <param name="list2">the second list.</param>
        /// <returns>the result list.</returns>
        public static List<TValue> operator -(List<TValue> list1, List<TValue> list2) => List.From(list1.ToFlat().Where(x => !(list2.ToFlat().ToList().Contains(x))));
